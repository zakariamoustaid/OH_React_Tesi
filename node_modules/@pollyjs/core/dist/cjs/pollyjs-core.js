...
class Logger {
  ...
  connect() {
    this._middleware = this.polly.server.any().on('error', (...args) => this.logError(...args)).on('response', (...args) => this.logRequest(...args));
  }
  ...
  logRequest(request) {
    if (request.config.logging) {
      ...
      console.groupCollapsed(`${FORMATTED_ACTIONS[request.action]} ➞ ${request.method} ${request.url} ${request.response.statusCode} • ${request.responseTime}ms`);
...
class Container {
  ...
  register(Factory) {
    ...
    utils.assert(`Invalid registration id provided. Expected string, received: "${typeof name}"`, typeof name === 'string');
...
function customizer(objValue, srcValue, key) {
  ...
  if (isArray$2(objValue) || ['context'].includes(key)) {
    return srcValue;
  }
}
...
function validateRequestConfig(config) {
  utils.assert(`Invalid config provided. Expected object, received: "${typeof config}".`, isObjectLike(config) && !isArray$2(config)); // The following options cannot be overridden on a per request basis
...
function validateTimesOption(times) {
  utils.assert(`Invalid number provided. Expected number, received: "${typeof times}".`, typeof times === 'number');
  utils.assert(`Invalid number provided. The number must be greater than 0, received "${typeof times}".`, times > 0);
}
...
const STOP_PROPAGATION = symbol$1();

class Event {
  constructor(type, props) {
    utils.assert(`Invalid type provided. Expected a non-empty string, received: "${typeof type}".`, type && typeof type === 'string');

    defineProperty$3(this, 'type', {
      value: type
...
class EventEmitter {
  ...
  constructor(options = {}) {
    ...
    utils.assert('An array of supported events must be provided via the `eventNames` option.', isArray$2(eventNames) && eventNames.length > 0);
  ...
  on(eventName, listener, options = {}) {
    ...
    if (times) {
      ...
      const tempListener = cancelFnAfterNTimes(listener, times, () => this.off(eventName, tempListener));
      /*
  ...
  once(eventName, listener, options = {}) {
    this.on(eventName, listener, objectSpread({}, options, {
      times: 1
    }));
    return this;
  }
  ...
  async emitParallel(eventName, ...args) {
    assertEventName(eventName, this[EVENT_NAMES]);
    const event = new Event(eventName);
    await promise$1.all(this.listeners(eventName).map(listener => listener(...args, event)));
...
class Interceptor extends Event {
  constructor() {
    super('intercept');
    setDefaults(this);
  }
...
class PollyRequest extends HTTPBase {
  constructor(polly, request) {
    ...
    */
    ...
    this.action = null; // Interceptor instance to be passed to each of the intercept handlers

    this._interceptor = new Interceptor(); // Lookup the associated route for this request

    this[ROUTE] = polly.server.lookup(this.method, this.url); // Filter all matched route handlers by this request

    this[ROUTE].applyFiltersWithArgs(this); // Handle config overrides defined by the route

    this.configure(this[ROUTE].config()); // Handle recording name override defined by the route
  ...
  get absoluteUrl() {
    ...
    return isAbsoluteUrl(url) ? url : new utils.URL(url).href;
  }
  ...
  async respond(response) {
    ...
    utils.assert('Cannot respond to a request that already has a response.', !this.didRespond);
    ...
    this.response.timestamp = utils.timestamp(); // Set the status code
    ...
    this.response.isBinary = isBinary; // Trigger the `beforeResponse` event

    await this._emit('beforeResponse', this.response); // End the response so it can no longer be modified
  ...
  async _identify() {
    ...
    keys$4(NormalizeRequest).forEach(key => {
      if (this[key] && matchRequestsBy[key]) {
        this.identifiers[key] = NormalizeRequest[key](this[key], matchRequestsBy[key], this);
      }
    }); // Emit the `identify` event which adapters can use to serialize the request body

    await this[EVENT_EMITTER].emit('identify', this); // Freeze the identifiers so they can no longer be modified

    freeze$1(this.identifiers); // Guid is a string representation of the identifiers

    this.id = md5(stringify(this.identifiers)); // Order is calculated on other requests with the same id and recording id
...
function requestWithParams(req, {
  params
}) {
...
class Route {
  ...
  config() {
    return mergeConfigs(...this[HANDLERS].map(({
      handler
    }) => handler.get('config')));
  }
  ...
  applyFiltersWithArgs(req, ...args) {
    this[HANDLERS] = this[HANDLERS].filter(({
      route,
      handler
    }) => [...handler.get('filters')].every(fn => fn(requestWithParams(req, route), ...args)));
  }
  /**
  ...
  async intercept(req, res, interceptor) {
    for (const _ref of this[HANDLERS]) {
      const {
        route,
        handler
      } = _ref;
      ...
      if (handler.has('intercept')) {
        await handler.get('intercept')(requestWithParams(req, route), res, interceptor);
      }
    }
  }
  /**
  ...
  async emit(eventName, req, ...args) {
    for (const _ref2 of this[HANDLERS]) {
      const {
        route,
        handler
      } = _ref2;
      const shouldContinue = await handler._eventEmitter.emit(eventName, requestWithParams(req, route), ...args);
  ...
  _valueFor(key) {
    ...
    for (const _ref3 of this[HANDLERS]) {
      const {
        handler
      } = _ref3;
...
class Handler extends map$1 {
  ...
  on(eventName, listener, options = {}) {
    this._eventEmitter.on(eventName, listener, objectSpread({}, this.get('defaultOptions'), options));
  ...
  intercept(fn, options = {}) {
    ...
    options = objectSpread({}, this.get('defaultOptions'), options);
  ...
  filter(fn) {
    utils.assert(`Invalid filter callback provided. Expected function, received: "${typeof fn}".`, typeof fn === 'function');
...
class Server {
  ...
  _register(method, routes) {
    ...
    castArray(routes).forEach(route => {
      const {
        host,
        path
      } = parseUrl$1(this._buildUrl(route));
  ...
  _registryForHost(host) {
    if (!this[REGISTRY][host]) {
      this[REGISTRY][host] = utils.HTTP_METHODS.reduce((acc, method) => {
...
const PAUSED_ADAPTERS = symbol$1();
...
class Polly {
  ...
  async stop() {
    if (this.mode !== utils.MODES.STOPPED) {
  ...
  async flush() {
    ...
    await promise$1.all( // The NOOP is there to handle both a resolved and rejected promise
    // to ensure the promise resolves regardless of the outcome.
    this._requests.map(r => promise$1.resolve(r.promise).then(NOOP, NOOP)));
  }
  /**
  ...
  connectTo(idOrAdapter) {
    ...
    utils.assert(`Adapter matching the name \`${adapterId}\` was not registered.`, container.has(`adapter:${adapterId}`));
...
async function afterEach(context, framework) {
  ...
  defineProperty$5(context, 'polly', {
    ...
    get() {
      throw new utils.PollyError(`You are trying to access an instance of Polly that is no longer available.\n` + `See: https://netflix.github.io/pollyjs/#/test-frameworks/${framework}?id=test-hook-ordering`);
    }
...
//# sourceMappingURL=pollyjs-core.js.map
