...
*/
import { ACTIONS, assert, timeout, MODES, EXPIRY_STRATEGIES, URL, HTTP_STATUS_CODES, timestamp, buildUrl, HTTP_METHODS, PollyError } from '@pollyjs/utils';
...
class Container {
  ...
  register(Factory) {
    ...
    assert(`Invalid registration id provided. Expected string, received: "${typeof name}"`, typeof name === 'string');
...
function validateRequestConfig(config) {
  assert(`Invalid config provided. Expected object, received: "${typeof config}".`, isObjectLike(config) && !isArray$2(config)); // The following options cannot be overridden on a per request basis
...
function validateTimesOption(times) {
  assert(`Invalid number provided. Expected number, received: "${typeof times}".`, typeof times === 'number');
  assert(`Invalid number provided. The number must be greater than 0, received "${typeof times}".`, times > 0);
}
...
class Event {
  constructor(type, props) {
    assert(`Invalid type provided. Expected a non-empty string, received: "${typeof type}".`, type && typeof type === 'string');
...
class EventEmitter {
  ...
  constructor(options = {}) {
    ...
    assert('An array of supported events must be provided via the `eventNames` option.', isArray$2(eventNames) && eventNames.length > 0);
...
class Handler extends map$1 {
  ...
  filter(fn) {
    assert(`Invalid filter callback provided. Expected function, received: "${typeof fn}".`, typeof fn === 'function');
...
export { Polly, Timing, setupMocha, setupQunit };
